...
describe('Trophées', () => {
	...
	test('trophée « Sans faute partout » : seuil dynamique = nb réel de leçons (#39)', () => {
		...
		expect(def.test!({ stars: 5, totalLessons: 5 } as GSnapshot)).toBe(true);
		expect(def.test!({ stars: 4, totalLessons: 5 } as GSnapshot)).toBe(false);
		// Ajout de leçons ⇒ le même nombre d'étoiles ne suffit plus.
		expect(def.test!({ stars: 67, totalLessons: 67 } as GSnapshot)).toBe(true);
