...
export function setActivePrefix(p){ activePrefix=p||''; }
...
export function setOnDataWrite(fn){ onDataWrite=fn; }
...
export function lsKeysRaw(){ const o=[]; try{for(let i=0;i<localStorage.length;i++) o.push(localStorage.key(i));}catch(e){} return o; }
export function lsRemoveRaw(realK){ try{localStorage.removeItem(realK);}catch(e){} }
export function lsSetRaw(realK,rawValue){ try{localStorage.setItem(realK,rawValue);}catch(e){} }
/* Toutes les clés de l'appli (tous profils confondus) */
export function appKeys(){ return lsKeysRaw().filter(k=>k.includes('ludaskia_')); }
