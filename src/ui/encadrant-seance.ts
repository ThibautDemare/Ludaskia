...
import {
	favorisProfil,
	favorisDispo,
	checkboxesFavorisHTML,
	gestionFavorisHTML,
	supprimerFavori,
} from './encadrant-favoris';
import {
	SEANCE_MODE_INFOS,
	chargerSeancesFor,
...
function ctxComposeur(uuid: string): ContexteSeance {
	return {
		...
		favorisDisponibles: favorisDispo(uuid),
	};
}
...
export function seanceClick(act: string, el: HTMLElement): boolean {
	...
	switch (act) {
		...
		case 'seance-favori-del': {
			// Le module des favoris ne connaît ni l'alerte de la carte ni le re-rendu : il
			// renvoie seulement si la suppression a eu lieu, et c'est ici qu'on en tire les
			// conséquences d'écran.
			void supprimerFavori(consulte, el.dataset.id ?? '').then((fait) => {
				if (!fait) return;
				alerte = null;
