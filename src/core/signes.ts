...
/* Mot-légende court (UN mot : tient sous le glyphe sans retour à la ligne) et
   ...
   pas « inférieur à »). */
const SIGNE_MOTS: Record<string, string> = { '<': 'petit', '=': 'égal', '>': 'grand' };
...
export function signeView(signe: string): ChoiceView {
	return {
		html:
			`<span class="lqcm-sym-glyph" aria-hidden="true">${escapeHTML(signe)}</span>` +
			`<span class="lqcm-sym-mot" aria-hidden="true">${escapeHTML(SIGNE_MOTS[signe] ?? signe)}</span>`,
		label: SIGNE_LABELS[signe] ?? signe,
	};
}
