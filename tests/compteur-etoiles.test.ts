...
describe('compteur d’étoiles — branche multi-niveaux (non-régression, critère 8)', () => {
	it('met en avant le cumul et passe l’objectif de la classe en sous-ligne', () => {
		expect(
			compteurEtoilesHTML(
				etat({ starsNiveau: 3, totalNiveau: 33, starsCumul: 8, labelClasse: 'CM1' }),
			),
		).toBe(
	...
	it('cumul d’une seule étoile : accords au singulier', () => {
		expect(
			compteurEtoilesHTML(
				etat({ starsNiveau: 0, totalNiveau: 33, starsCumul: 1, labelClasse: 'CM1' }),
			),
		).toBe(
	...
	it('zéro au niveau actif : l’objectif invite au lieu de noter (aucun « 0 sur N »)', () => {
		const html = compteurEtoilesHTML(
	...
	it('sous-ligne au singulier : l’accord suit le numérateur', () => {
		expect(
			compteurEtoilesHTML(
				etat({ starsNiveau: 1, totalNiveau: 33, starsCumul: 4, labelClasse: 'CM1' }),
			),
		).toBe(
	...
	it('la sous-ligne nomme les étoiles dans SES DEUX cas (avancée et invitation)', () => {
		...
		expect(sousLigne(compteurEtoilesHTML(etat({ starsNiveau: 12, starsCumul: 20 })))).toMatch(
			/étoiles?/,
		);
		expect(sousLigne(compteurEtoilesHTML(etat({ starsNiveau: 0, starsCumul: 20 })))).toMatch(
			/étoiles?/,
		);
	});
	...
	it('objectif à une seule leçon : « 1 étoile à gagner » au singulier', () => {
		const html = compteurEtoilesHTML(
			etat({ starsNiveau: 0, totalNiveau: 1, starsCumul: 4, labelClasse: 'CM1' }),
		);
...
describe('compteur d’étoiles — branche mono-niveau (critères 4 et 7)', () => {
	it('critère 7 : un enfant qui démarre ne lit jamais un « 0 sur N »', () => {
		const html = compteurEtoilesHTML(etat({ starsNiveau: 0, totalNiveau: 33, starsCumul: 0 }));
	...
	it('critère 4 : le compteur parle d’étoiles, plus de « leçons réussies sans faute »', () => {
		const html = compteurEtoilesHTML(etat({ starsNiveau: 12, totalNiveau: 33, starsCumul: 12 }));
	...
	it('accord au singulier avec une seule étoile', () => {
		const html = compteurEtoilesHTML(etat({ starsNiveau: 1, totalNiveau: 33, starsCumul: 1 }));
	...
	it('l’égalité cumul = étoiles du niveau relève bien de la branche mono-niveau', () => {
		...
		const mono = compteurEtoilesHTML(etat({ ...base, starsCumul: 5 }));
		const multi = compteurEtoilesHTML(etat({ ...base, starsCumul: 8 }));
...
describe('compteur d’étoiles — cas défensifs', () => {
	it('catalogue vide de la classe, mais des étoiles ailleurs : le trésor reste, l’objectif se tait', () => {
		...
		const html = compteurEtoilesHTML(
			etat({ starsNiveau: 0, totalNiveau: 0, starsCumul: 5, labelClasse: 'CM1' }),
		);
	...
	it('catalogue vide et aucune étoile : rien à afficher du tout', () => {
		...
		expect(compteurEtoilesHTML(etat({ starsNiveau: 0, totalNiveau: 0, starsCumul: 0 }))).toBe('');
	});
	...
	it('module pur : ni DOM ni stockage lus, sortie stable pour un même état', () => {
		...
		const a = compteurEtoilesHTML(e);
		expect(a).not.toBe('');
		expect(compteurEtoilesHTML(e)).toBe(a);
