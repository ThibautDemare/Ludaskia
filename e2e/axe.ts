/* ============================================================
   ...
   Périmètre du scan : WCAG 2.0/2.1/2.2 niveaux A + AA (les tags ci-dessous).
   ...
   cible d'une app enfant sur tablette. Le niveau 2.2 AA apporte notamment
   `target-size` (2.5.8, cibles tactiles ≥ 24 px), pertinent sur tablette.
   ============================================================ */
import AxeBuilder from '@axe-core/playwright';
import type { Page } from '@playwright/test';
import type { AxeResults, Result, NodeResult } from 'axe-core';

/* Tags axe correspondant à WCAG 2.0/2.1/2.2 niveaux A et AA. */
...
/* Aplati et tronque le `failureSummary` d'axe (multi-ligne, préfixé « Fix any of
   the following: ») pour donner, en une ligne, le POURQUOI actionnable — surtout
   le ratio mesuré et les couleurs pour `color-contrast`. */
const MAX_SUMMARY = 200;
function summarizeFailure(summary?: string): string {
	if (!summary) return '';
	const flat = summary
		.replace(/^Fix (any|all) of the following:/i, '')
		.replace(/\s+/g, ' ')
		.trim();
...
/* Formate le résultat axe d'UNE vue en un bloc texte lisible, REGROUPÉ PAR RÈGLE
   puis par élément (critère #411). Trié par sévérité décroissante. Destiné à être
   lu tel quel dans les logs CI par un humain OU un agent — sans décorticage. Rend
   aussi les `incomplete` (règles qu'axe n'a pas pu trancher, ex. `target-size` sur
   des éléments qui se chevauchent) : à vérifier à la main, à ne pas perdre. */
export function formatA11yReport(view: string, hash: string, results: AxeResults): string {
	...
	const lines = [`[axe] ${view} (#${hash}) — ${violations.length} violation(s)`];

	if (violations.length === 0) {
		lines.push('  OK : aucune violation WCAG A/AA détectée.');
	} else {
		lines[0] += ` · ${impactSummary(violations)}`;
		for (const v of violations) {
			...
			for (const node of v.nodes.slice(0, MAX_ELEMENTS_PAR_REGLE)) {
				...
				const why = summarizeFailure(node.failureSummary);
	...
	if (incomplete.length) {
		lines.push(`  [à vérifier manuellement] ${incomplete.length} règle(s) — axe n'a pas tranché :`);
		for (const inc of incomplete) {
			lines.push(`    ~ ${inc.id} (${inc.nodes.length} élément(s)) — ${inc.help}`);
		}
	}
